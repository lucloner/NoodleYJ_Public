...
function initInfo() {
    ...
    $("#head h5").html("与 " + info.username + " 通讯中 (" + info.ip + "/" + info.group + ")");
    $("#head p").html("该设备 <u>" + (info.isabsence ? "离开" : "在线") + "</u>"
        + (info.isabsence ? " (" + info.statemsg + ")" : "") + "，" + (info.enc ? "对方支持加密通讯" : "对方不支持加密通讯，消息可能会被嗅探")
        + "，双击标签页关闭此窗口。<br />欢迎使用，如果您有任何建议或问题，请邮件告知昱瑾科技：<a href=\"#\" onclick=\"window.external.GoUrl(\'mailto:root@biggee.cn\');\">root@biggee.cn</a>。");
    host = info;
    $(".umsg input.openPackage").live("click", openPackage);
    savedHEADp = $("#head p").html();
}
$(initInfo);

/**
 * 添加提示信息
 * @param {Object} msg 要显示信息内容
 */
...
function fileSendingPre(isDirectory, path, size) {
    ...
    if (obj.length == 0) {
        ...
        html.push('<h5>准备发送文件</h5><div class="list"></div><p><input type="button" class="send" value="确定发送" />&nbsp;<input type="button" class="cancel" value="取消发送" /></p></div>');
...
function passReceived(task, saveToSameLocation) {
    //空,提供自定义
}
function shotnregReceived(task, saveToSameLocation) {
    //空,提供自定义
}

function showPics(filenames) {
    ct--;
    var html2add = "";
    var html_head_p = $("#head p").html();
    if (html_head_p.indexOf("<IMG  id=showPics ") > -1) {
        html2add = $("#head p").html();
        clearTimeout(timeoutHEADp);
        timeoutHEADp = setTimeout('removePics();', 3000);
    } else {
        timeoutHEADp=setTimeout('removePics();', 3000);
    }
    ...
    for (var i = 0; i < filenames.length; i++) {
        ...
        if (html_head_p != savedHEADp) {
            if (html_head_p.indexOf(filenames[i]) > -1) {
                html2add += "!";
                continue;
            }
        }
        if (html2add.indexOf(filenames[i]) > -1) {
            html2add += "|";
            continue;
        }
        html2add += '<img id=showPics height=100 src="' + filenames[i] + '" />';
    }
    $("#head p").html(html2add);
}

function removePics() {
    var s = $("#head p").html().indexOf("<IMG id=showPics ");
    if (s > -1) {
        //alert($("#head p").html());
        $("#head p").html(savedHEADp);        
    }
}
...
function fileRecvStateChange(pkgid, idx, filename, curName, state) {
    ...
    if (state == 3) {
        applyTipInfo("<strong>" + curName + "</strong> 接收失败，请稍后重试。");
    }
    else if (state == 4) {
        applyTipInfo("<strong>" + curName + '</strong> 接收成功。');//<img width="160px" src="' + curName + '">
        //applyTipInfo("<textarea rows=100 cols=200>" + document.body.parentElement.innerHTML + "</textarea>");
    }
}
...
function ctrltabs(tcmd, tid, who) {
    who.setAttribute("disabled", true);
    var html2add = "<button onclick=yj_face_rename({id2bind},this)>改名</button><button onclick=yj_face_delete({id2bind},this)>删除</button>";
    addButtons(tid, "卡号", html2add);
}

function yj_face_rename(idBinded,who) {
    who.setAttribute("disabled", true);
    var name = prompt("请输入新的值", idBinded); //将输入的内容赋给变量 name
    window.external.YjFaceRen(idBinded, name);
    alert("改名已下发,请重新读取列表\n由于表格为异步,再次改名须重新读取");
}

function yj_face_delete(idBinded,who) {
    who.setAttribute("disabled", true);
    if (confirm("删除后将不可逆,你确定吗?\n点击确定继续")) {
        window.external.YjFaceDel(idBinded);
    } else {
        who.removeAttribute("disabled");
    }
}
...
function exp2xls(tableid, cmd) {//哎,一失足成千古恨,抄了一个ext2xls忘了改了
    HtmlExportToExcelForIE(gettablehtml(tableid), "EXCEL_" + cmd + tableid);
}
...
function HtmlExportToExcelForIE(tableid, filename) {
    try {
        ...
        var strHTML = tableid;
        var reg2rep = new RegExp("<TR style=\"DISPLAY: none\">","g");
        strHTML = strHTML.replace(reg2rep, "<TR style=\"DISPLAY: block\">");
        var headStart = strHTML.indexOf("<TR");
        var headEnd = strHTML.indexOf("</TR>");
        headEnd = headEnd + 5;
        var headHTML = strHTML.substring(headStart, headEnd);
        var strHTML1 = strHTML.substring(0, headEnd);
        var strHTML2 = strHTML.substring(headEnd);
        reg2rep = new RegExp(headHTML, "g");
        strHTML2 = strHTML2.replace(reg2rep, "");
